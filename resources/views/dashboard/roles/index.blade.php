...
<div class="content">
	<div class="card">
		...
		<div class="card-body">
			<button type="button" style="background-color:#26a69a; border-color: #26a69a"  class="btn btn-success" data-toggle="modal" data-target="#ModalRolesForm" > <i class="fa fa-plus"></i> Add Roles</button>
			<div class="row">
					...
					<table class="table table-responsive datatable-column-search-selects">
						...
						<tbody>
							...
							<tr id="drivers2" style="cursor: pointer;">
								<td onclick="getRole({{ $role->id }})">
									{{ $role->id }}
								</td>
								<td onclick="getRole({{ $role->id }})">
									{{ $role->name }}
								</td>
								<td onclick="getRole({{ $role->id }})">{{ str_replace(array('[',']','"'),'', $role->permissions()->pluck('name')) }}</td>{{-- Retrieve array of permissions associated to a role and convert to string --}}
										...
										<div class="list-icons">
											<div class="dropdown">
												...
												<div class="dropdown-menu dropdown-menu-right">
													<a class="dropdown-item" onclick="deleteRole({{ $role->id }})"><i class="fas fa-trash"></i> Delete </a>
												</div>
											</div>
										</div>
									</td>
	...
	<script type="text/javascript">
		$("#createRole").click(function(e){
			...
			jQuery.ajax({
				...
				{
					swal('Role'+result.id, "has been created successfully!", "success").then(function(){ 
		...
		$("#editRole").click(function(e){
			...
			var name = $("#Name").val()
			var permissions = [];
			$.each($("input[name='permissionsedit[]']:checked"), function(){
				permissions.push($(this).val());
			});
			var id = $("#idNumber").val()
			...
			jQuery.ajax({
				...
				success:function(result)
				{
					swal('Role'+result.id, "has been updated successfully!", "success").then(function(){ 
						window.location.reload()
					}
					);
				},
			...
			.then((willDelete) => {
				if (willDelete) {
					jQuery.ajax({
						...
						success:function(result)
						{
							swal('Role'+result.id, "has been deleted!", {
								icon: "success",
							}).then(function(){ 
								window.location.reload()
							}
					...
					swal("Role is safe!").then(function(){ 
						window.location.reload()
					}
					);
				}
