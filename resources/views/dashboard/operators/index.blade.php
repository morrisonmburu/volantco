...
<div class="card">
	<div class="card-header header-elements-inline">
		<h5 class="card-title">Operators Table</h5>
	...
	<div class="card-body">
		<button type="button" style="background-color:#26a69a; border-color: #26a69a"  class="btn btn-success" data-toggle="modal" data-target="#ModalUserForm" > <i class="fa fa-plus"></i> Add Operator</button> 
		<table class="table table-responsive datatable-column-search-selects">
			...
			<tbody id="table">
				...
				<tr id="drivers2" style="cursor: pointer;">
					<td onclick="getOperator({{ $user->id }})">
						{{ $user->id }}
					</td>
					<td onclick="getOperator({{ $user->id }})">
						{{ $user->name }}
						...
						<div class="list-icons">
							<div class="dropdown">
								...
								<div class="dropdown-menu dropdown-menu-right">
									<a class="dropdown-item" onclick="deleteOperator({{ $user->id }})"><i class="fas fa-trash"></i> Delete </a>
								</div>
							</div>
						</div>
					</td>
...
@include("dashboard.operators.create")
@include("dashboard.operators.edit")

</div>


<script type="text/javascript">
	$("#createOperator").click(function(e){
		e.preventDefault()
		var name = $("#name").val()
		var email = $("#email").val()
		var roles = [];
		...
		jQuery.ajax({
			...
			method:"POST",
			data:{name:name, email: email, roles: roles},
					success:function(result)
					{
						swal('Operator'+result.id, "has been created successfully!", "success").then(function(){ 
	...
	$("#editOperator").click(function(e){
		...
		jQuery.ajax({
			...
			url:'{{ route("operators.update") }}',
			method:"POST",
			data:{id: id, name:name, email: email, roles: roles},
					success:function(result)
					{
						swal('Operator'+result.id, "has been updated successfully!", "success").then(function(){ 
							window.location.reload()
						}
						);
					},
	...
	function deleteOperator(id){
				swal({
					title: "Are you sure?",
					text: "Once deleted, you will not be able to recover this operator!",
					icon: "warning",
					buttons: true,
					dangerMode: true,
				})
				.then((willDelete) => {
					if (willDelete) {
						jQuery.ajax({
							url:'{{ route('operators.destroy') }}',
							method:"POST",
							data:{id: id, _token: '{{csrf_token()}}'},
							success:function(result)
							{
								swal('Operator'+result.id, "has been deleted!", {
									icon: "success",
								}).then(function(){ 
									window.location.reload()
								}
						...
						swal("Operator is safe!").then(function(){ 
							window.location.reload()
						}
						);
					}
